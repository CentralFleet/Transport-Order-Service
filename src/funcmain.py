...
class TransportOrders:
    ...
    def parse_release_forms(self, vehicles: List[Dict[str, str]]) -> List[str]:
        try:
            release_forms = [
                manage_prv(v["ReleaseForm"])
                for v in vehicles
                if v.get("ReleaseForm") not in ["null", None, ""]
            ]
    ...
    def create_order_db(self, session: any, body: Dict[str, any]) -> Optional[OrdersDB]:
        try:
            order_obj = OrdersDB(
    ...
    def create_order_zoho(self, order_obj: OrdersDB, body: Dict[str, any], token: str) -> Optional[int]:
        try:
            order_data = Order(
                ...
                Tax_Province=body.get("Dropoff_Province", "").replace("é", "e"),
                Pickup_Province=body.get("Pickup_Province", "").replace("é", "e"),
            ...
            response = ZOHO_API.create_record(
                moduleName="Deals", data={"data": [order_data.dict()]}, token=token
            )
    ...
    def create_vehicles_in_zoho(self, vehicles: List[Dict[str, any]], deal_id: int, token: str, order_obj: OrdersDB) -> Dict[str, any]:
        try:
            layout_info = {
            ...
            response = ZOHO_API.create_record(
                moduleName="Vehicles", data={"data": vehicles}, token=token
            )
    ...
    def attach_files(self, deal_id: int, file_urls: List[str], token: str) -> None:
        for url in file_urls:
            try:
                ...
                response = ZOHO_API.attach_file(
                    moduleName="Deals", record_id=deal_id, file_path=local_file_path, token=token
                )
    ...
    async def _create_order(self, body: Dict[str, any]) -> Dict[str, any]:
        try:
    ...
    async def _update_sql_order(self, body: Dict[str, any]) -> Dict[str, any]:
        try:
            with DatabaseConnection(connection_string=os.getenv("SQL_CONN_STR")) as session:
    ...
    async def _update_order(self, body: Dict[str, any]) -> Dict[str, any]:
        try:
            token = TOKEN_INSTANCE.get_access_token()
            body_filtered = body.copy()  # Ensure we don't modify the original data
            body_filtered.pop("DealID", None)  # Remove DealID if it exists

            update_response = ZOHO_API.update_record(
                moduleName="Deals", id=body.get("DealID", ""), data={"data": [body_filtered]}, token=token
            )
